...
/*
	Reads in data from the user and uses it to construct a config file that Sentinel View can use
	in the future.
...
func SetupCommand(c *cli.Context) error {
	...
	for {
		...
		data.SENTINELVIEWSettings.PiHoleAddress = ip.String()
		break
	}
	...
	for {
		...
		if len(trimmed) > 0 {
			...
			testAddressWithPort := network.GenerateAPIAddress(data.SENTINELVIEWSettings.PiHoleAddress, intPiHolePort)
			if network.IsAlive(testAddressWithPort) {
				data.SENTINELVIEWSettings.PiHolePort = intPiHolePort
			} else {
				continue
			}
		} else {
			testAddressWithPort := network.GenerateAPIAddress(data.SENTINELVIEWSettings.PiHoleAddress, data.DefaultPort)
			if network.IsAlive(testAddressWithPort) {
				data.SENTINELVIEWSettings.PiHolePort = data.DefaultPort
			} else {
				continue
			}
		}
	...
	for {
		...
		if len(trimmed) > 0 {
			...
			data.SENTINELVIEWSettings.RefreshS = intRefreshS
			break
		} else {
			break
		}
	...
	for {
		...
		data.SENTINELVIEWSettings.APIKey = apiKey
		...
		if !auth.ValidateAPIKey(data.SENTINELVIEWSettings.APIKey) {
	...
	if storageChoice == "y" || len(storageChoice) == 0 {
		err := auth.StoreAPIKeyInKeyring(data.SENTINELVIEWSettings.APIKey)
	...
	if err := data.SENTINELVIEWSettings.SaveToFile(); err != nil {
